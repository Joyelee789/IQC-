...
def calculate_inspector_category_efficiency(filtered_df, inspector_name):
    """
    計算單一檢驗員在各物料大類別的效率
    ...
    for _, row in inspector_data.iterrows():
        ...
        category_stats[main_category]['total_standard_time'] += standard_time
        category_stats[main_category]['total_actual_time'] += actual_time
        category_stats[main_category]['record_count'] += 1
        category_stats[main_category]['subcategories'].add(subcategory)
    
    # 計算各大類別的效率並整理成列表
    result = []
    for main_category, stats in category_stats.items():
        if stats['total_actual_time'] > 0:
            efficiency = min(stats['total_standard_time'] / stats['total_actual_time'], MAX_EFFICIENCY)
        else:
            efficiency = 0
        ...
        result.append({
            ...
            'efficiency': efficiency,
            'total_standard_time': stats['total_standard_time'],
            'total_actual_time': stats['total_actual_time'],
...
def render_efficiency_dashboard(efficiency_data, processed_data=None):
    ...
    with subtab2:
        ...
        <div style="background: linear-gradient(135deg, #667eea20 0%, #764ba220 100%); padding: 15px 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #667eea;">
            <p style="margin: 0; color: #333; font-size: 14px;">
                <strong>💡 個人深度分析</strong>：選擇一位檢驗員，查看其各物料大類別的檢驗效率表現與時間趨勢變化。
            </p>
        </div>
        """, unsafe_allow_html=True)
        
        # 獲取所有檢驗員列表（按總效率排序）
        inspector_list = overall_efficiency_ranking['inspector'].tolist()
        ...
        if inspector_list:
            ...
            with col1:
                selected_inspector = st.selectbox(
                    "選擇檢驗員",
                    options=inspector_list,
                    key="inspector_selector_for_category_analysis",
            ...
            with col2:
                st.info("💡 顯示選定檢驗員在各物料大類別的效率表現，識別優勢與改善空間")
            
            if selected_inspector:
                # 計算該檢驗員的各物料大類別效率
                inspector_category_data = calculate_inspector_category_efficiency(processed_data, selected_inspector)
                ...
                if inspector_category_data:
                    ...
                    # 計算平均效率（用於參考線）
                    avg_efficiency = df_inspector_cat['efficiency'].mean()
                    ...
                    # 為每個類別設定顏色（紅藍配色）
                    colors = []
                    for eff in df_inspector_cat['efficiency']:
                        if eff < 1:
                    ...
                    # 添加橫向條形圖（反轉順序使最高效率在最上方）
                    fig.add_trace(go.Bar(
                        y=df_inspector_cat['category'][::-1],  # 反轉順序
                        x=df_inspector_cat['efficiency'][::-1],
                        orientation='h',
                        marker_color=colors[::-1],
                        text=df_inspector_cat['efficiency'][::-1].round(2),
                        textposition='outside',
                    ...
                    # 設定圖表佈局
                    max_efficiency_display = min(5, df_inspector_cat['efficiency'].max() * 1.2)
                    fig.update_layout(
                        title=f"{selected_inspector} - 各物料大類別檢驗效率",
                        xaxis_title="檢驗效率",
                        yaxis_title="物料大類別",
                        height=max(400, len(df_inspector_cat) * 50),  # 根據類別數量調整高度
                        xaxis_range=[0, max_efficiency_display],
                        showlegend=False,
                        hovermode='closest'
                    )
                    
                    st.plotly_chart(fig, use_container_width=True, key=f"inspector_category_chart_{selected_inspector}")
                    
                    # 顯示詳細數據表格 - 使用 checkbox 而非 expander 避免巢狀
                    st.markdown("---")
                    show_details = st.checkbox("📊 顯示詳細數據", key=f"show_details_{selected_inspector}")
                    
                    if show_details:
                        # 準備表格數據
                        detail_table = df_inspector_cat.copy()
                        detail_table['效率'] = detail_table['efficiency'].round(2)
                        detail_table['標準工時(分鐘)'] = detail_table['total_standard_time'].round(0)
                        detail_table['實際耗時(分鐘)'] = detail_table['total_actual_time'].round(0)
                        detail_table['檢驗筆數'] = detail_table['record_count']
                        detail_table['包含子類別'] = detail_table['subcategories']
                        
                        # 選擇要顯示的欄位
                        display_table = detail_table[['category', '效率', '標準工時(分鐘)', '實際耗時(分鐘)', '檢驗筆數', '包含子類別']]
                        display_table.columns = ['物料大類別', '效率', '標準工時(分鐘)', '實際耗時(分鐘)', '檢驗筆數', '包含子類別']
                        
                        # 添加效率狀態標籤
                        def get_efficiency_status(eff):
                            if eff >= 1.2:
                                return '🟢 優秀'
                            elif eff >= 1.0:
                                return '🔵 良好'
                        ...
                        display_table['狀態'] = display_table['效率'].apply(get_efficiency_status)
                        
                        # 應用樣式
                        def highlight_row_by_efficiency(row):
                            eff = row['效率']
                            if eff < 1:
                                red_intensity = max(70, 255 - int((1 - eff) * 180))
                                color = f'background-color: rgba({red_intensity}, 0, 0, 0.2)'
                            else:
                                blue_intensity = max(70, 255 - int((eff - 1) * 50))
                                color = f'background-color: rgba(0, 0, {blue_intensity}, 0.2)'
                            return [color] * len(row)
                        
                        styled_table = display_table.style.apply(highlight_row_by_efficiency, axis=1)
                        ...
                        with col2:
                            max_eff_category = df_inspector_cat.iloc[0]['category']
                            max_eff_value = df_inspector_cat.iloc[0]['efficiency']
                            st.metric("最佳類別", max_eff_category, f"{max_eff_value:.2f}")
                        with col3:
                            min_eff_category = df_inspector_cat.iloc[-1]['category']
                            min_eff_value = df_inspector_cat.iloc[-1]['efficiency']
                            st.metric("待改進類別", min_eff_category, f"{min_eff_value:.2f}")
                        with col4:
                            total_records = df_inspector_cat['record_count'].sum()
                            st.metric("總檢驗筆數", f"{total_records}")
                    ...
                    # 計算該檢驗員的效率趨勢數據
                    trend_data = calculate_inspector_efficiency_trend(processed_data, selected_inspector)
                    
                    if trend_data is not None and not trend_data.empty:
                        # 建立趨勢選擇
                        trend_mode = st.radio(
                            "趨勢顯示模式",
                            options=["整體效率趨勢", "各材料類別趨勢"],
                            horizontal=True,
                            key=f"trend_mode_{selected_inspector}"
                        )
                        
                        if trend_mode == "整體效率趨勢":
                            # 繪製整體效率趨勢圖
                            fig_trend = go.Figure()
                            ...
                            fig_trend.add_trace(go.Scatter(
                                ...
                                marker=dict(size=6),
                                hovertemplate='<b>%{x}</b><br>效率: %{y:.2f}<br>檢驗筆數: %{customdata}<extra></extra>',
                                customdata=trend_data['record_count']
                            ))
                            ...
                            # 添加移動平均線（7天）
                            if len(trend_data) >= 7:
                                trend_data['ma7'] = trend_data['efficiency'].rolling(window=7, min_periods=1).mean()
                                fig_trend.add_trace(go.Scatter(
                                    x=trend_data['date'],
                                    y=trend_data['ma7'],
                                    mode='lines',
                            ...
                            fig_trend.update_layout(
                                title=f"{selected_inspector} - 檢驗效率趨勢",
                                xaxis_title="日期",
                                yaxis_title="檢驗效率",
                                height=400,
                                hovermode='x unified',
                            ...
                            st.plotly_chart(fig_trend, use_container_width=True, key=f"trend_overall_{selected_inspector}")
                        
                        else:
                            # 繪製各材料類別趨勢圖
                            category_trend_data = calculate_inspector_category_trend(processed_data, selected_inspector)
                            
                            if category_trend_data is not None and not category_trend_data.empty:
                                # 獲取所有類別
                                categories = category_trend_data['category'].unique().tolist()
                                
                                # 選擇要顯示的類別
                                selected_trend_categories = st.multiselect(
                                    "選擇要顯示的材料類別",
                                    options=categories,
                                    default=categories[:min(3, len(categories))],  # 預設顯示前3個
                                    key=f"trend_categories_{selected_inspector}"
                                )
                                ...
                                if selected_trend_categories:
                                    ...
                                    # 配色方案
                                    colors = ['#5B7C99', '#ffa726', '#66bb6a', '#ef5350', '#ab47bc', '#29b6f6', '#8d6e63']
                                    
                                    for i, cat in enumerate(selected_trend_categories):
                                        cat_data = category_trend_data[category_trend_data['category'] == cat]
                                        fig_cat_trend.add_trace(go.Scatter(
                                            x=cat_data['date'],
                                            y=cat_data['efficiency'],
                                            mode='lines+markers',
                                            name=cat,
                                    ...
                                    fig_cat_trend.update_layout(
                                        title=f"{selected_inspector} - 各材料類別效率趨勢",
                                        xaxis_title="日期",
                                        yaxis_title="檢驗效率",
                                        height=450,
                                        hovermode='x unified',
                                    ...
                                    st.plotly_chart(fig_cat_trend, use_container_width=True, key=f"trend_category_{selected_inspector}")
                                else:
                                    st.info("請選擇至少一個材料類別")
                            else:
                                st.info("無法計算各材料類別的趨勢數據")
                    else:
                        st.info("該檢驗員的趨勢數據不足")
                        
                else:
                    st.warning(f"查無 {selected_inspector} 的物料類別效率資料")
        else:
            st.info("請先處理資料以查看個別人員分析")
    ...
    with subtab3:
        st.markdown("""
        <div style="background: linear-gradient(135deg, #66bb6a20 0%, #42a5f520 100%); padding: 15px 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #66bb6a;">
            <p style="margin: 0; color: #333; font-size: 14px;">
                <strong>💡 物料類別分析</strong>：選擇物料大類別或子類別，比較各檢驗員在特定物料類別的效率表現。
            </p>
        </div>
        """, unsafe_allow_html=True)
        
        st.write("📋 **選擇分析模式與物料類別**")
        ...
        if 'has_applied_selection' in st.session_state and st.session_state.has_applied_selection:
            ...
            if selected_subcategories:
                if selected_category_mode == "category" or merge_categories:
                    ...
                    if merged_category_key in category_efficiency_data:
                        ...
                        # 計算顏色（使用漸層配色，與團隊總覽一致）
                        colors = []
                        for eff in df_sorted['efficiency']:
                            if eff < 0.8:
                                colors.append('#ef5350')  # 紅色（低效率）
                            elif eff < 1.0:
                                colors.append('#ffa726')  # 橙色
                        ...
                        fig.add_trace(go.Bar(
                            ...
                            text=[f'{e:.2f}' for e in df_sorted['efficiency']],
                            textposition='outside',
                            hovertemplate='<b>%{y}</b><br>效率: %{x:.2f}<br>檢驗筆數: %{customdata}<extra></extra>',
                            customdata=df_sorted['record_count']
                        ))
                        ...
                        # 設置圖表佈局
                        fig.update_layout(
                            title=f"📦 {merged_category} 類別效率排名",
                            xaxis_title="檢驗效率",
                            yaxis_title="",
                            height=max(400, len(df_sorted) * 35),
                            showlegend=False,
                            xaxis_range=[0, max_display_value],
                        ...
                        # 顯示類別效率數據明細 - 使用 checkbox 避免巢狀
                        st.markdown("---")
                        if st.checkbox("📋 顯示類別效率數據明細", key="show_merged_category_details"):
                    ...
                    for i, category in enumerate(selected_subcategories):
                        if category in category_efficiency_data:
                            ...
                            # 顯示類別效率數據明細 - 使用 checkbox 避免巢狀
                            st.markdown("---")
                            if st.checkbox(f"📋 顯示 {category} 類別效率數據明細", key=f"show_category_details_{i}"):
                                display_category_details(df)
                        else:
            ...
            # 如果用戶還沒有應用選擇，顯示友善的提示
            st.markdown("""
                <div style="background-color:#e3f2fd; border-left:4px solid #2196f3; border-radius:4px; padding:16px; margin-top:20px;">
                    <p style="margin:0; color:#1565c0; font-weight:400;">👆 請先選擇分析模式和物料類別，然後點擊「應用選擇」按鈕查看效率排名。</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            for cat in valid_cats:
                ...
                if median_eff > 1.5:
                    ...
                    recommendations.append({
                        ...
                        '樣本數': count,
                        '判定': '🔴 標準過鬆',
                        '建議': f'建議下修標準工時，係數約 {adjust_factor:.2f}'
                    })
                elif median_eff > 1.3:
                    adjust_factor = 1 / median_eff
                    recommendations.append({
                        ...
                        '樣本數': count,
                        '判定': '🟠 略微偏鬆',
                        '建議': f'可考慮微調標準工時，係數約 {adjust_factor:.2f}'
                    })
                elif median_eff < 0.5:
                    adjust_factor = 1 / median_eff
                    recommendations.append({
                        ...
                        '樣本數': count,
                        '判定': '🔵 標準過緊',
                        '建議': f'建議上調標準工時，係數約 {adjust_factor:.2f}'
                    })
                elif median_eff < 0.7:
                    adjust_factor = 1 / median_eff
                    recommendations.append({
...
def render_quality_speed_matrix(processed_data, efficiency_data):
    ...
    <div style="background: linear-gradient(135deg, #667eea20 0%, #764ba220 100%); padding: 15px 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #667eea;">
        <p style="margin: 0; color: #333; font-size: 14px;">
            ...
            高效率但低MRB開立率的人員可能存在「放水」風險，需要重點關注。
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    # 計算中位數作為象限分界線
    efficiency_median = inspector_stats['平均效率'].median()
    ...
    with col_q2:
        risk_count = quadrant_counts.get('⚠️ 高風險區', 0)
        st.metric("⚠️ 高風險區", f"{risk_count} 人", 
                 delta="需關注" if risk_count > 0 else None,
                 delta_color="inverse",
                 help="高效率 + 低MRB開立率（可能放水）")
    ...
    # 右上
    fig.add_annotation(
        x=inspector_stats['平均效率'].max() * 0.9,
    ...
    # 左上
    fig.add_annotation(
        x=inspector_stats['平均效率'].min() + 0.1,
        y=inspector_stats['MRB開立率'].max() * 0.9,
        text="💪 苦幹區",
        showarrow=False,
    ...
    if selected_person:
        ...
        with col_d4:
            st.metric("平均效率", f"{person_summary['平均效率']:.2f}")
        
        col_d5, col_d6, col_d7, col_d8 = st.columns(4)
        
        with col_d5:
            st.metric("中位數效率", f"{person_summary['中位數效率']:.2f}")
        with col_d6:
        ...
        with col_hist:
            ...
            fig_hist.add_vline(x=person_summary['平均效率'], line_dash="dash", line_color="red",
                              annotation_text=f"平均({person_summary['平均效率']:.2f})")
        ...
        with col_cat:
            ...
            fig_cat.add_trace(go.Bar(
                ...
                orientation='h',
                marker_color='#5C8AE6',
                text=cat_stats['MRB率'].apply(lambda x: f'{x:.1f}%'),
                textposition='outside'
            ))
            fig_cat.update_layout(
                title=f"{selected_person} 各類別MRB開立率",
                xaxis_title="MRB開立率 (%)",
                yaxis_title="",
                height=350
            )
        ...
        # 檢驗明細表格
        with st.expander(f"📋 {selected_person} 檢驗明細（最近100筆）", expanded=False):
            display_cols_qsm = ['類別', '料號', '檢驗日期', '處理後檢驗標準工時', '檢驗耗時', '效率比值', '有MRB']
            available_cols_qsm = [col for col in display_cols_qsm if col in person_detail.columns]
            
            show_df_qsm = person_detail[available_cols_qsm].sort_values('檢驗日期', ascending=False).head(100).copy()
            
            if '檢驗日期' in show_df_qsm.columns:
                show_df_qsm['檢驗日期'] = pd.to_datetime(show_df_qsm['檢驗日期'], errors='coerce').dt.strftime('%Y-%m-%d')
            if '效率比值' in show_df_qsm.columns:
                show_df_qsm['效率比值'] = show_df_qsm['效率比值'].round(2)
            if '有MRB' in show_df_qsm.columns:
                show_df_qsm['有MRB'] = show_df_qsm['有MRB'].apply(lambda x: '✅' if x else '❌')
            
            show_df_qsm.columns = ['類別', '料號', '日期', '標準工時(分)', '實際耗時(分)', '效率', 'MRB']
            
            st.dataframe(show_df_qsm, use_container_width=True, hide_index=True)
    ...
    with q_tab1:
        gold_df = inspector_stats[inspector_stats['象限分類'] == '🥇 金牌檢驗員'][
            ['處理後檢驗員', '檢驗批數', 'MRB批數', 'MRB開立率', '平均效率', '中位數效率']
    ...
    with q_tab2:
        risk_df = inspector_stats[inspector_stats['象限分類'] == '⚠️ 高風險區'][
            ['處理後檢驗員', '檢驗批數', 'MRB批數', 'MRB開立率', '平均效率', '中位數效率']
        ].sort_values('MRB開立率', ascending=True)
        
        if not risk_df.empty:
            st.markdown("**⚠️ 警示：** 這些人員效率高但MRB開立率低，需要關注是否有「放水」情況")
            st.dataframe(risk_df, use_container_width=True, hide_index=True)
        else:
            st.success("✅ 目前沒有人員在高風險區")
    ...
    with q_tab4:
        need_df = inspector_stats[inspector_stats['象限分類'] == '📚 需輔導區'][
            ['處理後檢驗員', '檢驗批數', 'MRB批數', 'MRB開立率', '平均效率', '中位數效率']
        ].sort_values('平均效率', ascending=True)
...
def main():
    ...
    if st.session_state.files_uploaded:
        ...
        # Dock 導航 HTML/CSS/JS (類似 Framer Motion 效果)
        dock_nav_html = """
        <style>
        .dock-wrapper {
            display: flex;
        ...
        .dock-item:hover .dock-icon-wrapper {
            ...
            box-shadow: 0 12px 28px rgba(96, 125, 139, 0.4);
        }
        ...
        /* 鄰居放大效果 */
        .dock-item.neighbor-1 .dock-icon-wrapper {
            transform: scale(1.25) translateY(-4px);
        }
        
        .dock-item.neighbor-2 .dock-icon-wrapper {
        ...
        <div class="dock-wrapper">
            <div class="dock-container" id="dockContainer">
                <div class="dock-item {active_0}" data-index="0" onclick="selectTab(0)">
                    <span class="dock-label">檢驗效率監控</span>
                    <div class="dock-icon-wrapper {active_icon_0}">
                        <span class="dock-icon">⚡</span>
                    </div>
                    <div class="dock-indicator"></div>
                </div>
                <div class="dock-item {active_1}" data-index="1" onclick="selectTab(1)">
                    <span class="dock-label">異常偵測</span>
                    <div class="dock-icon-wrapper {active_icon_1}">
                        <span class="dock-icon">🕵️</span>
                    </div>
                    <div class="dock-indicator"></div>
                </div>
                <div class="dock-item {active_2}" data-index="2" onclick="selectTab(2)">
                    <span class="dock-label">效率品質矩陣</span>
                    <div class="dock-icon-wrapper {active_icon_2}">
                        <span class="dock-icon">🎯</span>
                    </div>
                    <div class="dock-indicator"></div>
                </div>
                <div class="dock-item {active_3}" data-index="3" onclick="selectTab(3)">
                    <span class="dock-label">工作負載監控</span>
                    <div class="dock-icon-wrapper {active_icon_3}">
                        <span class="dock-icon">⏱️</span>
                    </div>
                    <div class="dock-indicator"></div>
                </div>
                <div class="dock-item {active_4}" data-index="4" onclick="selectTab(4)">
                    <span class="dock-label">時間分配監控</span>
                    <div class="dock-icon-wrapper {active_icon_4}">
                        <span class="dock-icon">⚖️</span>
                    </div>
                    <div class="dock-indicator"></div>
                </div>
                <div class="dock-item {active_5}" data-index="5" onclick="selectTab(5)">
                    <span class="dock-label">額外任務監控</span>
                    <div class="dock-icon-wrapper {active_icon_5}">
                        <span class="dock-icon">📋</span>
                    </div>
                    <div class="dock-indicator"></div>
                </div>
        ...
        // Tab 選擇功能 - 透過 Streamlit 通訊
        function selectTab(index) {
            // 使用 Streamlit 的 setComponentValue 或透過隱藏 input
            const event = new CustomEvent('streamlit:setComponentValue', {
                detail: { value: index }
            });
            window.dispatchEvent(event);
            
            // 方法2: 透過 URL 參數觸發 Streamlit 重新載入
            const currentUrl = new URL(window.parent.location.href);
            currentUrl.searchParams.set('dock_tab', index);
        ...
        dock_html = dock_nav_html.format(
            active_0='active' if current_tab == 0 else '',
            active_1='active' if current_tab == 1 else '',
            active_2='active' if current_tab == 2 else '',
            active_3='active' if current_tab == 3 else '',
            active_4='active' if current_tab == 4 else '',
            active_5='active' if current_tab == 5 else '',
            active_icon_0='active' if current_tab == 0 else '',
            active_icon_1='active' if current_tab == 1 else '',
            active_icon_2='active' if current_tab == 2 else '',
            active_icon_3='active' if current_tab == 3 else '',
            active_icon_4='active' if current_tab == 4 else '',
            active_icon_5='active' if current_tab == 5 else '',
        )
        
        st.markdown(dock_html, unsafe_allow_html=True)
        
        # 使用按鈕列作為備用導航（實際觸發 tab 切換）
        st.markdown("""
        <style>
        .dock-buttons-row {
            display: flex;
        ...
        .dock-buttons-row .stButton button {
        ...
        .dock-buttons-row .stButton button:hover {
            opacity: 1;
        ...
        div[data-testid="column"] .stButton button {
            ...
            border: 1px solid rgba(120, 144, 156, 0.3) !important;
        ...
        # 備用 Streamlit 按鈕（透過點擊觸發 tab 切換）
        # 使用不同的視覺樣式標示當前選中
        current = st.session_state.current_dock_tab
        
        # 添加當前選中按鈕的高亮樣式
        st.markdown(f"""
        <style>
        div[data-testid="stHorizontalBlock"] > div:nth-child({current + 1}) .stButton button {{
        ...
        with col1:
            if st.button("⚡ 效率", key="dock_btn_0", use_container_width=True):
                st.session_state.current_dock_tab = 0
                st.rerun()
        with col2:
            if st.button("🕵️ 異常", key="dock_btn_1", use_container_width=True):
                st.session_state.current_dock_tab = 1
                st.rerun()
        with col3:
            if st.button("🎯 矩陣", key="dock_btn_2", use_container_width=True):
                st.session_state.current_dock_tab = 2
                st.rerun()
        with col4:
            if st.button("⏱️ 負載", key="dock_btn_3", use_container_width=True):
                st.session_state.current_dock_tab = 3
                st.rerun()
        with col5:
            if st.button("⚖️ 分配", key="dock_btn_4", use_container_width=True):
                st.session_state.current_dock_tab = 4
                st.rerun()
        with col6:
            if st.button("📋 任務", key="dock_btn_5", use_container_width=True):
                st.session_state.current_dock_tab = 5
                st.rerun()
        ...
        elif st.session_state.current_dock_tab == 4:
            render_time_allocation_dashboard(st.session_state.time_allocation_data)
        elif st.session_state.current_dock_tab == 5:
